...
class GradienceApplication(Adw.Application):
	...
	def do_activate(self):
		...
		self.create_action("restore_color_scheme", self.show_restore_color_scheme_dialog)
		...
		if response == "reset":
			...
			if widget.get_app_types()["gtk3"]:
				...
				try:
					...
					pass
			self.win.toast_overlay.add_toast(Adw.Toast(title=_("Preset reset")))
