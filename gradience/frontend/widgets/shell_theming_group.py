...
from gradience.frontend.dialogs.unsupported_shell_version_dialog import (
    GradienceUnsupportedShellVersionDialog,
)
...
class GradienceShellThemingGroup(Adw.PreferencesGroup):
    ...
    def reload_colors(self, *args):
        try:
            ...
            logging.error(
                "An unexpected error occurred while loading variable colors.", exc=e
            )
            self.toast_overlay.add_toast(
                Adw.Toast(
                    title=_(
                        "An unexpected error occurred while loading variable colors."
                    )
                )
            )
    ...
    def on_custom_colors_button_clicked(self, *_args):
        self.shell_pref_window = GradienceShellPrefsWindow(
            self.parent, self.shell_colors
        )
    ...
    def on_apply_button_clicked(self, *_args):
        if not is_gnome_available():
            dialog = Adw.MessageDialog(
                ...
                heading=_("GNOME Shell Are Missing"),
                body=_(
                    "This Theme Engine is designed to work only on systems running GNOME. You can still generate themes on other desktop environments, but it won't have any affect on them."
                ),
            )
            ...
            dialog.set_response_appearance(
                "disable-engine", Adw.ResponseAppearance.DESTRUCTIVE
            )
    ...
    def apply_shell_theme(self):
        ...
        try:
            ShellTheme().apply_theme_async(
                self, self._on_shell_theme_done, variant_str, self.app.preset
            )
            ...
            self.toast_overlay.add_toast(
                Adw.Toast(title=_("An error occurred while generating a Shell theme."))
            )

    def _on_shell_theme_done(
        self,
        source_widget: GObject.Object,
    ...
    def on_shell_missing_response(self, widget, response, *args):
        if response == "disable-engine":
            ...
            enabled_engines = GLib.Variant.new_strv(
                list(self.win.enabled_theme_engines)
            )
