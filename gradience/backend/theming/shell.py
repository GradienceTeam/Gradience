...
class ShellTheme:
    ...
    def set_shell_theme(self):
        if is_sandboxed():
            schema_id = self.THEME_GSETTINGS_SCHEMA_ID
            key = self.THEME_GSETTINGS_SCHEMA_KEY

            def _on_reset_finish(_subprocess):
                # Set theme generated by Gradience
                self.settings.write_value_async(None, schema_id, key, "gradience-shell")

            # Set default theme
            self.settings.reset_value_async(_on_reset_finish, schema_id, key)
        else:
            # Set default theme
    ...
    def reset_shell_theme(self):
        if is_sandboxed():
            ...
            # Set default theme
            self.settings.reset_value_async(None, schema_id, key)
        else:
            # Set default theme
            self.settings.set_string("name", "")
